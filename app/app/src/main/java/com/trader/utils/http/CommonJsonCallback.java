...
public class CommonJsonCallback implements Callback {
        ...
        mDeliveryHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (!Utils.isConnected(MyApplication.context)) {
                    mListener.onFailure(new OhException(NETWORK_ERROR, "请检查网络"));
                } else if (e instanceof SocketTimeoutException) {
                    //判断超时异常
                    mListener.onFailure(new OhException(TIMEOUT_ERROR, "请求超时"));
                } else if (e instanceof ConnectException) {
                    //判断超时异常
                    mListener.onFailure(new OhException(OTHER_ERROR, "请求服务器失败"));
                } else {
                    mListener.onFailure(new OhException(NETWORK_ERROR, e.getMessage()));
                }
                */
    ...
    private void handleResponse(Object responseObj) {
        if (responseObj == null && responseObj.toString().trim().equals("")) {
            mListener.onFailure(new OhException(NETWORK_ERROR, NETWORK_MSG));
            return;
        }
                ...
                if (result.getInt(RESULT_CODE) == RESULT_CODE_VALUE) {
                    ...
                    if (mClass == null) {
                        ...
                        if (obj != null) {
                            ...
                            mListener.onFailure(new OhException(JSON_ERROR, JSON_MSG));
                        }
                    }
                } else { //将服务端返回的异常回调到应用层去处理
                    //mListener.onFailure(new OkHttpException(OTHER_ERROR, result.get(ERROR_MSG) + ""));
                    mListener.onFailure(new OhException(result.getInt(RESULT_CODE), result.getString("msg")));
                    Log.e("TAG", "onResponse处理失败");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            mListener.onFailure(new OhException(OTHER_ERROR, e.getMessage()));
